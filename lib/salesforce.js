...
export function parseFlowLog(logBody) {
  ...
  for (const line of lines) {
    ...
    // Format: timestamp|FLOW_START_INTERVIEW_BEGIN|guid|flowName
    if (line.includes('FLOW_START_INTERVIEW_BEGIN|')) {
      const parts = line.split('|');
      const guid = parts[2] || '';
      const flowName = parts[3] || 'Unknown Flow';
      interviews[guid] = {
        guid,
        flowName,
        startTime: timestamp,
    ...
    // FLOW_ELEMENT_BEGIN
    // Format: timestamp|FLOW_ELEMENT_BEGIN|guid|elementType|elementName
    if (line.includes('FLOW_ELEMENT_BEGIN|')) {
      const parts = line.split('|');
      const guid = parts[2] || '';
      const elementType = parts[3] || '';
      const elementName = parts[4] || '';
      const element = {
        type: 'ELEMENT_BEGIN',
        guid,
        elementType,
    ...
    // FLOW_ELEMENT_ERROR
    // Format: timestamp|FLOW_ELEMENT_ERROR|message (no guid)
    if (line.includes('FLOW_ELEMENT_ERROR|')) {
      const parts = line.split('|');
      const message = parts.slice(2).join('|');
      // Associate with the most recent interview
      const guids = Object.keys(interviews);
      const lastGuid = guids[guids.length - 1] || '';
      if (interviews[lastGuid]) {
        interviews[lastGuid].status = 'error';
        interviews[lastGuid].errorMessage = message;
      }
      events.push({ type: 'ELEMENT_ERROR', guid: lastGuid, message, timestamp, nanos });
    }
    ...
    if (line.includes('FLOW_ELEMENT_FAULT|')) {
      ...
      events.push({ type: 'ELEMENT_FAULT', guid: lastGuid, message, timestamp, nanos });
    }

    // FLOW_VALUE_ASSIGNMENT
    // Format: timestamp|FLOW_VALUE_ASSIGNMENT|guid|varName|varValue
    if (line.includes('FLOW_VALUE_ASSIGNMENT|')) {
      const parts = line.split('|');
      const guid = parts[2] || '';
      const varName = parts[3] || '';
      const varValue = parts.slice(4).join('|').substring(0, 200); // truncate long values
      if (interviews[guid]) {
